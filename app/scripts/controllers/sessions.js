...
angular.module('cCheckApp')
  .controller('AddSessionCtrl', ['$scope', 'fighters', 'sessions', '$q', function ($scope, fighters, sessions, $q) {
    ...
    $scope.fighters = fighters.query(); 
    $q.all([$scope.sessions.$promise, $scope.fighters.$promise]).then(function(response) {
            ...
            $scope.fighters.forEach(function(fighter) {
                ...
                $scope.sessions.forEach(function(session) {
                    var result = $.grep(session.full_attendance, function(entry) {return entry === fighter.id})
                    if(result.length === 0) {
                        // not full attendance
                        if($.grep(session.half_attendance, function(entry) {return entry === fighter.id}).length === 1) {
                            // half attendance
                            fighter.total += 0.5;
                        }                    
                    }
